...
class BodyModal extends React.Component {
  ...
  getPopUpArray = (display) =>{
    ...
    topicsToDisplay.forEach((topic) => {
      ...
      bodys.forEach((body) => {
        ...
        popUpArray.push(<div><p><b>{subjectArrayToDisplay}<br/></b>{bodyArrayToDisplay}</p>{outerTextToDisplay}</div>);
      });
    });
    return popUpArray;
  }
  ...
  getsubjectArray = (display) =>{
    ...
    var subjectArray = [];
    var topicsToDisplay = display;
    var listOfTopicsTest = [];

    topicsToDisplay.forEach((topic) => {
      var bodys = topic.body;
      //var subject = body.subject;
      subjectArray.push(<div><h1>{topic.name}</h1></div>);
      var k = 0;
      bodys.forEach((body) => {
        var bodyArray = body.text.split(/(\[\[|\]\]|\n)/g);
        var subject = body.subject.split(/(\[\[|\]\]|\n)/g);
        ...
        subjectArray.push(<div><p><b>{subjectArrayToDisplay}<br/></b>{bodyArrayToDisplay}</p>{outerTextToDisplay}</div>);
      });
    });
    return subjectArray;
  }
  ...
  getOuterText = (button) => {
    console.log(this.props.getTopic(button));
